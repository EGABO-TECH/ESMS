...
import { Settings, Shield, BookOpen, GraduationCap, Save, UserCheck, User } from "lucide-react";
        ...
        <div className="xl:col-span-1 space-y-2">
          {sections.map((section) => (
            <button key={section.id} onClick={() => setActiveTab(section.id)}
              className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all text-left group border ${activeTab === section.id ? "bg-blue-600 text-white border-blue-600 shadow-lg" : "bg-white text-slate-600 border-slate-200 hover:border-blue-600/50"}`}
            >
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 space-y-8">
            <div className="flex items-center justify-between pb-6 border-b border-slate-100">
              <h2 className="text-xl font-bold text-slate-900 capitalize">
                {activeTab === "profile" ? "Personal Account" : `${activeTab} Policy`}
              </h2>
              {activeTab !== "profile" && (
                <button onClick={() => toast.success("Registry policies updated")}
                  className="px-6 py-2 bg-blue-600 text-white rounded-xl font-bold text-sm shadow-md hover:opacity-90 transition-all flex items-center gap-2">
            ...
            {activeTab === "profile" && (
              <div className="space-y-2">
                ...
                <div className="pt-4"><ProfileSettingsForm accentColor="blue" /></div>
              </div>
            )}

            {activeTab !== "academic" && activeTab !== "profile" && (
